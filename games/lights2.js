export function init(){
  ...
  canvas.addEventListener("touchend",function(e){
    ...
    grid[c] = 1 - grid[c];
    if(c%round != 0 && (c-round) >= 0){
      grid[c-1-round] = 1 - grid[c-1-round];
    }
    if((c+1)%round != 0 && (c+round) < round*round){
      grid[c+1+round] = 1 - grid[c+1+round];
    }
    if((c-round) >= 0 && (c+1)%round != 0){
      grid[c-round+1] = 1 - grid[c-round+1];
    }
    if((c + round) < round*round && (c%round) != 0){
      grid[c+round-1] = 1 - grid[c+round-1];
    }
